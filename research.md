...
with [Juan Pablo Torres-Martínez](https://econ.uchile.cl/es/academico/jutorres){:target="_blank" rel="noopener"} <br> 
 <font size = "4"><button onclick="myFunction('abstract')" class="btn--research">`Abstract`</button> [`SSRN`](http://dx.doi.org/10.2139/ssrn.4276906){:target="_blank"} [`Slides`](../files/research/Weak_Externalities_School_Choice/slides.pdf){:target="_blank"} [`Slides (spanish)`](../files/research/Weak_Externalities_School_Choice/slides_esp.pdf){:target="_blank" rel="noopener"}</font>
